...
public:
  ...
  // Gazebo update-thread callback for deferred physics operations.
  void OnUpdate();
  ...
  bool detach_done_ = false;
  JointSTRUCT pending_detach_joint_;
...
void GazeboLinkAttacherPrivate::OnUpdate()
{
...

  {
    ...
    joint_to_detach = pending_detach_joint_;
    detach_pending_ = false;
  }

  RCLCPP_INFO(
    ros_node_->get_logger(),
    "DETACH DEBUG: Gazebo update thread about to call Joint::Detach()"
  );

  joint_to_detach.joint->Detach();

  RCLCPP_INFO(
    ros_node_->get_logger(),
    "DETACH DEBUG: Gazebo update thread Joint::Detach() completed"
  );
  ...
  linkattacher_msgs::srv::DetachLink::Response::SharedPtr _res)
{
  ...
  if (this->getJoint(_req->model1_name, _req->link1_name, _req->model2_name, _req->link2_name, j)){
    RCLCPP_INFO(
      ros_node_->get_logger(),
      "DETACH DEBUG: joint found, queueing detach for Gazebo update thread"
    );

    {
      std::lock_guard<std::mutex> lock(detach_mutex_);
      pending_detach_joint_ = j;
      detach_done_ = false;
    ...
    if (!detach_completed) {
      _res->success = false;
      _res->message =
        "DETACH ERROR: Gazebo update-thread detach timed out.";

      RCLCPP_ERROR(
        ros_node_->get_logger(),
        "DETACH DEBUG: Gazebo update-thread detach timed out"
      );
    ...
    RCLCPP_INFO(
      ros_node_->get_logger(),
      "DETACH DEBUG: service callback confirmed update-thread detach completed"
    );

    RCLCPP_INFO(
      ros_node_->get_logger(),
      "DETACH DEBUG: about to write response success"
    );
    ...
    RCLCPP_INFO(
      ros_node_->get_logger(),
      "DETACH DEBUG: response success written, about to write response message"
    );
    ...
    RCLCPP_INFO(
      ros_node_->get_logger(),
      "DETACH DEBUG: response message written, about to erase bookkeeping entry"
    );
    ...
    // IMPORTANT:
    // Do not call Model::RemoveJoint() here.
    // On this Gazebo 11 setup it causes gzserver to segfault
    // immediately after Joint::Detach().
    //
    // The joint is already physically detached above. Remove
    // only our bookkeeping entry so a stale JointPtr is not
    // reused by a later DETACH request.
    for (auto it = GV_joints.begin(); it != GV_joints.end(); ++it) {
      if (
        it->model1 == _req->model1_name &&
    ...
    RCLCPP_INFO(
      ros_node_->get_logger(),
      "DETACH DEBUG: detached joint bookkeeping entry removed safely"
    );

    RCLCPP_INFO(
      ros_node_->get_logger(),
      "DETACH DEBUG: about to set IsAttached=false"
    );
    ...
    RCLCPP_INFO(
      ros_node_->get_logger(),
      "DETACH DEBUG: IsAttached=false completed, about to return from Detach callback"
    );

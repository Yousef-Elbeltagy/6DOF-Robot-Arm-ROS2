...
def main():
    ...
    node = Node("sequence_action_test")
    node.set_parameters(
    ...
    if not action_client.wait_for_server(timeout_sec=10.0):
        print("FAILED: /sequence_move_group is unavailable.")
        rclpy.shutdown()
        return
    ...
    try:
        ...
        print(f"FAILED to load saved poses: {error}")
        rclpy.shutdown()
        return
    ...
    for name in sequence_names:
        if name not in saved_poses:
            print(f'FAILED: Pose "{name}" does not exist.')
            rclpy.shutdown()
            return
        ...
        if joints is None or len(joints) != 6:
            print(
                f'FAILED: Pose "{name}" does not contain '
                "six saved joint positions."
            )
            rclpy.shutdown()
            return
    ...
    while rclpy.ok():
        ...
        input(
            f"\nPress ENTER to send run {run_number} "
            "(Ctrl+C to stop)..."
        )
        ...
        print(f"[RUN {run_number}] before send_goal_async()")

        send_future = action_client.send_goal_async(goal)

        print(
            f"[RUN {run_number}] send_goal_async() returned future"
        )
        ...
        print(
            f"[RUN {run_number}] SEND FUTURE DONE"
        )
        ...
        if goal_handle is None:
            print(
                f"[RUN {run_number}] FAILED: goal handle is None"
            )
            continue
        ...
        print(
            f"[RUN {run_number}] waiting for result future..."
        )
        ...
        print(
            f"[RUN {run_number}] RESULT FUTURE DONE"
        )

        wrapped_result = result_future.result()

        print(
            f"[RUN {run_number}] action status = "
            f"{wrapped_result.status}"
        )

        print(
            f"[RUN {run_number}] MoveIt error code = "
